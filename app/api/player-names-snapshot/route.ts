...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    return NextResponse.json(
      {
        ...
        marketsCaptured: snapshot.marketsCaptured,
        playerNamesBySport: snapshot.playerNamesBySport,
    ...
    console.error('Player names snapshot generation failed', {
      error: error instanceof Error ? error.message : String(error),
    });

    return NextResponse.json({ error: 'Player names snapshot failed. Check server logs for details.' }, { status: 500 });
  }
}
