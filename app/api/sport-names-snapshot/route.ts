...
import { createSportNamesSnapshot } from '@/lib/nameSnapshots';
...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    const snapshot = await createSportNamesSnapshot(options, apiKey);
    ...
    return NextResponse.json(
      {
        ...
        sportNames: snapshot.sportNames,
    ...
    console.error('Sport names snapshot generation failed', {
      error: error instanceof Error ? error.message : String(error),
    });

    return NextResponse.json({ error: 'Sport names snapshot failed. Check server logs for details.' }, { status: 500 });
  }
}
